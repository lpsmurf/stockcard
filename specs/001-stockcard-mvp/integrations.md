...
| **Stripe Issuing (Sandbox)** | Test cards + authorizations | ✅ Self-serve (sandbox enabled Sept 15 under `acct_1UFwaXDI76S6oWGD`) | `sk_test_` / `pk_test_`, webhook `whsec_` | Virtual test cards, dashboard "Create test purchase", test-helper authorizations, synchronous `issuing_authorization.request` webhook (2 s), Issuing Elements for card details | **Stretch provider (T029c); mock stays default** |
| **Bridge SEPA off-ramp** | USDC → EUR to IBAN | 💬 Sales-gated | `Api-Key`; EUR/SEPA access via sales@bridge.xyz; customer needs the SEPA endorsement | External account (`account_type: "iban"`), liquidation address on Solana USDC routing to `payment_rail: "sepa"` (SEPA Instant + SEPA Credit), `developer_fee_percent` for our fee | **Mock in MVP; production path** |
| **Frankfurter (ECB rates)** | EUR/USD reference | ✅ Yes | None | Daily ECB reference rates for payout quotes | **Yes, payout quotes** |
