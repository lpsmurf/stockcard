...
| Sources (Pyth) | `GET https://pyth.dourolabs.app/hermes/v2/updates/price/latest?ids[]=0x…&parsed=true`, header `Authorization: Bearer PYTH_API_KEY` (required since 26 Aug 2026). Feed ids are resolved at runtime from `/v2/price_feeds?query={TICKER}&asset_type=equity` matching `Equity.US.{TICKER}/USD` (never the 24/7 `Equity.Index.…` variant), cached 24 h, overridable with `PYTH_FEED_IDS`. No Pyth feed exists for SPCX (pre-IPO). Reading is used only if `publish_time` is within 5 min and `conf/price` ≤ 100 bps |
| Pyth entitlement (checked Sept 16) | A free Hermes key reads **metadata but not equity prices**: `/v2/price_feeds` answers 200, `/v2/updates/price/latest` answers **403 `Not entitled: … asset type 'equity'`** (the same key returns 200 for crypto, e.g. BTC/USD). Equity prices need a Pyth Pro grant — which is what the Stocklana Pyth bounty awards. The signer treats 403 as "no Pyth price" and carries on with xStocks + Jupiter |
| Pyth market hours (works without Pro) | The free metadata carries `market_hours.is_open` / `next_open` from Pyth's own NYSE calendar, including holidays and half-days. The signer prefers it over the xStocks `openNow` flag to decide when a market is closed, and re-reads it every 60 s (the feed id is cached for 24 h, the open/closed flag never is). Verified Sept 16: NVDAx/SPYx/TSLAx `closed`, next open 13:30 UTC |
