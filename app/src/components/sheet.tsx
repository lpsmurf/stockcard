...
export function Sheet({ title, children, onClose, wide }: { title: string; children: ReactNode; onClose?: () => void; wide?: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-ink/40 md:items-center" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <div
        ref={ref}
        className={`relative w-full max-w-md rounded-t-2xl bg-surface p-5 pb-8 shadow-xl md:rounded-2xl ${wide ? "md:max-w-[560px]" : "md:max-w-[480px]"}`}
