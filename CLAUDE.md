...
## Session hygiene and ownership

See `AGENTS.md` → "Session hygiene and token budget" and "Who does what". Short version: one task per session; never open lock files, IDL json, `target/`, `.next/`, `node_modules/`, `test-ledger/` or validator logs; grep spec sections instead of reading whole files; pipe build output through `tail`; start the local validator only with `scripts/validator.sh`. Claude owns non-UI code (API routes, libs, program, scripts, specs); Kimi owns screens and components.
